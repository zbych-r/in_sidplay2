...
bool Player::config(const SidConfig &cfg, bool force)
{
    ...
    if (m_tune != nullptr)
    {
        ...
        try
        {
            ...
            sidCreate(cfg.sidEmulation, cfg.defaultSidModel, cfg.digiBoost, cfg.forceSidModel, addresses, cfg.secondSidModel, cfg.forceSecondSidModel);
                        ...
                        SidConfig::sid_model_t secondSidModel, bool forceSecondSidModel)
{
    if (builder != nullptr)
    {
        ...
        if (extraSidAddresses.size() != 0)
        {
            ...
            {
                unsigned int currentAddr = extraSidAddresses[i];
                SidConfig::sid_model_t userModel;
                //allow user to select second sid model
                if ((i == 0) && (secondSidModel > -1) && (forceSecondSidModel))
                {
                    //getSidModel(tuneInfo->sidModel(0), defaultModel, forced);
                    userModel = (SidConfig::sid_model_t)secondSidModel;
                }
                else
                {
