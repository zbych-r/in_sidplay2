...
{
    unsigned int sids = 1;
    for (auto esidBank: extraSidBanks)
        sids += esidBank.second->installedSIDs();

    if (sidBank.getShadowSID() != nullptr)
    {
        sids++;
    }
    return sids;
...
void c64::setShadowSid(c64sid* s)
{
    sidBank.setShadowSID(s);
}
