...
private:
    ...
    c64sid *sid;
    /**
    * secondarychip for pseudostereo
    */
    c64sid* shadowSid;
public:
    SidBank()
      : sid(NullSid::getInstance()), shadowSid(nullptr)
    {}

    void reset()
    {
        sid->reset();
        if (shadowSid) shadowSid->reset();
    }
    ...
    {
        sid->poke(addr, data);
        if (shadowSid) shadowSid->poke(addr, data);
    }
    ...
    void setShadowSID(c64sid* s) { shadowSid = s; }

    c64sid* getShadowSID() const { return shadowSid; }
};
