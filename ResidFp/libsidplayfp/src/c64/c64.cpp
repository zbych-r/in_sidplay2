...
void c64::removeParallelSid()
{
	sidBank.setParallelSID(NULL);
}
...
bool c64::addExtraSid(c64sid *s, int address)
{
	...
	//ZR: parallel SID for pseudostereo
	if (address == 0xD400)
	{
		sidBank.setParallelSID(s);
		//because this extra SID has the same address as base sid we can't allow it to be added to mapper
		return true;
	}
	else
	{
