...
/***************************************************************************
 * $Log: ini.cpp,v $
 * Revision 1.16  2009/08/13 06:54:22  s_a_white
 * Sync to latest libini to remove warnings under VC2005.
 *
...
#if defined HAVE_MSWINDOWS
	{
		ini->tempFileName = _tempnam(NULL,NULL);
		ini->ftmp = fopen(ini->tempFileName,"wb+");
	}
#else
        ini->ftmp = tmpfile ();
#endif
...
#if defined HAVE_MSWINDOWS
	if(ini->tempFileName != NULL)
	{
		remove(ini->tempFileName);
		free(ini->tempFileName);
	}
#endif
        if (ini->filename)
            free (ini->filename);
